			...
			<ul class="ul_news">
				...
				<c:forEach items="${noticelist}" var="notice" varStatus="loop">
					<c:if test="${!empty articlelist }">
       				 <li><span>${articlelist[loop.count-1].releasetime}</span> <a href="<%=basePath %>noticeController/noticeArtice.do?ntid=${notice.noticetype.ntid}&aid=${notice.article.aid}&pageNow=${pageNow }">${notice.nname }</a></li>
       				</c:if>
       			</c:forEach>
				</c:if>

...
<body>
	...
	<table id="noticetype_dg" class="easyui-datagrid" style="height: 670px;"
		url="noticeTypeController/findAll.do" toolbar="#noticetype_toolbar" pagination="true"
	...
	<script type="text/javascript">
		...
		function newNoticetype() {
			...
			url = 'noticeTypeController/add.do';
		...
		function editNoticetype() {
			...
			if (row) {
				...
				url = 'noticeTypeController/add.do?ntid=' + row.ntid;
			}
		...
		function destroyNoticetype() {
			...
			if (row) {
				$.messager.confirm('Confirm', '确定要删除吗?', function(r) {
					if (r) {
						$.post('noticeTypeController/delete.do', {
							ntid : row.ntid,
							nt:row.nt
						}, function(result) {
							if (result.success) {

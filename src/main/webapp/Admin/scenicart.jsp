...
<body>
	...
	<table id="scenicart_dg" class="easyui-datagrid" style="height: 500px;"
		url="scenicController/findAll.do" toolbar="#scenicart_toolbar" pagination="true"
	...
	<div id="scenicart_dlg" class="easyui-dialog" style="padding: 10px 20px" margin-top:50px;
		...
		<form id="scenicart_fm" method="post" novalidate enctype="multipart/form-data">
			...
			<div class="fitem">
			  ...
			  <br>
			<div class="fitem">
				<label>上传图片:</label><input name="file" id="imgPicker1" type="file"  onchange="xmTanUploadImg(this)">
			</div>
	...
	<script type="text/javascript">
		...
		function editscenic() {
			...
			if (row) {
				...
				url = 'scenicController/add.do?sid=' + row.sid+'&aid='+row.aid+'&';
		...
		function destroyscenic() {
			...
			if (row) {
				$.messager.confirm('Confirm', '确定要删除吗?', function(r) {
					if (r) {
						$.post('scenicController/delete.do?aid='+row.aid+"&", {
							sid : row.sid,
							image:row.image,
							lx:row.lx,
							sname:row.sname

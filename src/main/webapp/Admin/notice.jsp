...
<body>
	...
	<table id="notice_dg" class="easyui-datagrid" style="height: 670px;"
		url="noticeController/findAll.do" toolbar="#notice_toolbar" pagination="true"
	...
	<script type="text/javascript">
		...
		function editnotice() {
			...
			if (row) {
				...
				url = 'noticeController/add.do?nid=' + row.nid+'&aid='+row.aid;
			}
			getnoticetypelist();
		...
		function destroynotice() {
			...
			if (row) {
				$.messager.confirm('Confirm', '确定要删除吗?', function(r) {
					if (r) {
						$.post('noticeController/delete.do?aid='+row.aid+"&", {
							nid : row.nid,
							nname:row.nname
						}, function(result) {
							if (result.success) {

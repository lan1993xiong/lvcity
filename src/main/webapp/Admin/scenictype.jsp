...
<body>
	...
	<table id="scenictype_dg" class="easyui-datagrid" style="height: 500px;"
		url="scenictypeController/findAll.do" toolbar="#scenictype_toolbar" pagination="true"
	...
	<script type="text/javascript">
		...
		function newScenictype() {
			...
			url = 'scenictypeController/add.do';
		...
		function editScenictype() {
			...
			if (row) {
				...
				alert('666');
				url = 'scenictypeController/add.do?stid=' + row.stid;
			}
		...
		function destroyScenictype() {
			...
			if (row) {
				$.messager.confirm('Confirm', '确定要删除吗?', function(r) {
					if (r) {
						$.post('scenictypeController/delete.do', {
							stid : row.stid
						}, function(result) {
							if (result.success) {
								$('#scenictype_dg').datagrid('reload'); // reload the scenictype data

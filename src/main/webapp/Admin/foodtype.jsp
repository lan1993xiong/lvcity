...
<body>
   <!-- 
	...
	<table id="foodtype_dg" class="easyui-datagrid" style="height: 670px;"
		url="foodTypeController/findAllFoodType.do" toolbar="#foodtype_toolbar" pagination="true"
	...
	<script type="text/javascript">
		...
		function newfoodtype() {
			...
			url = 'foodTypeController/addFoodType.do';
		}
		function editfoodtype() {
			...
			if (row) {
				...
				url = 'foodTypeController/addFoodType.do?ftid=' + row.ftid;
			}
		}
		...
		function destroyfoodtype() {
			...
			if (row) {
				$.messager.confirm('Confirm', '确定要删除吗?', function(r) {
					if (r) {
						$.post('foodTypeController/delete.do?', {
							ftid : row.ftid,
							ft:row.ft
						}, function(result) {
							if (result.success) {

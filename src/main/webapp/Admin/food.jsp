...
<body>
	...
	<table id="food_dg" class="easyui-datagrid" style="height: 670px;"
		url="foodController/findAllFood.do" toolbar="#food_toolbar" pagination="true"
	...
	<script type="text/javascript">
		...
		function editfood() {
			...
			if (row) {
				...
				url = 'foodController/addFood.do?fid=' + row.fid+"&fname="+row.fname;
		...
		function destroyfood() {
			...
			if (row) {
				$.messager.confirm('Confirm', '确定要删除吗?', function(r) {
					if (r) {
						$.post('foodController/deleteFood.do',
						{
						     fid:row.fid,
						     fname:row.fname,
						     image:row.image
